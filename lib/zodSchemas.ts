...
export const fechaNacimientoSchema = z.string().refine((date) => {
  if (!date || date === '') return true; // Permitir vacío inicialmente
  ...
  return edad >= 18;
}, 'Debes seleccionar una fecha de nacimiento válida y ser mayor de edad');

// Validaciones para dirección
export const municipioSchema = z.number().min(1, 'Debes seleccionar un municipio');
